    ...
    const requestPromise = new Promise(async (resolve, reject) => {
        try {
            ...
            if (!forceRefresh) {
                ...
                if (cacheDoc.exists) {
                    ...
                    if (!isExpired && !emptyDataExpired) {
                        console.log(`Cache hit for ${cacheKey} - Age: ${Math.round(cacheAge)}s, Has data: ${hasData}`);
                        ...
                        if (emptyDataExpired) {
                            console.log(`Cache hit but empty data for ${cacheKey}, age: ${Math.round(cacheAge)}s, refreshing...`);
                        } else {
                            console.log(`Cache expired for ${cacheKey}, age: ${Math.round(cacheAge)}s, TTL: ${effectiveTTL}s`);
                        }
                    }
                } else {
            ...
            if (parseInt(dailyRemaining) < 100 || parseInt(minuteRemaining) < 5) {
                console.warn(`⚠️ API Rate Limit Warning - Daily: ${dailyRemaining}/${dailyLimit}, Minute: ${minuteRemaining}/${minuteLimit}`);
            }
