...
export default function TeamPage() {
  ...
  const transfersData = useMemo(() => {
    ...
    // 현재 팀과 관련된 이적만 필터링
    const teamTransfers = allTransfersData.transfers.filter((transfer: any) => {
      const fromClub = transfer.fromClub || transfer.from?.name || ''
      const toClub = transfer.toClub || transfer.to?.name || ''
      
      // 팀 이름이나 ID로 매칭
      const isRelated = 
        fromClub.toLowerCase().includes(teamName.toLowerCase()) ||
        toClub.toLowerCase().includes(teamName.toLowerCase()) ||
      ...
      // 방향 설정
      const toClub = transfer.toClub || transfer.to?.name || ''
      const isIn = toClub.toLowerCase().includes(teamName.toLowerCase()) || 
                   transfer.to?.id === teamId ||
                   transfer.toClubId === teamId
  ...
  }, [allTransfersData, teamId, profileData?.team?.name])
