...
serve(async (req) => {
  ...
  try {
    ...
    switch (endpoint) {
      case 'information':
        apiUrl += `/teams?id=${params.id}`
        break
      case 'statistics':
        apiUrl += `/teams/statistics?team=${params.team}&season=${params.season}&league=${params.league}`
        break
      case 'squad':
        apiUrl += `/players/squads?team=${params.team}`
        break
      case 'head-to-head':
        apiUrl += `/fixtures/headtohead?h2h=${params.h2h}`
        break
      default:
        return new Response(
